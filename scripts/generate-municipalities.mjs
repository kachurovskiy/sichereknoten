...
const workbookPath = path.join(root, "docs/data/AuszugGV2QAktuell.xlsx");
...
for (const row of rows) {
  ...
  if (rowType === "20" && hasValue(row.C) && hasValue(row.D) && hasValue(row.H)) {
    ...
    municipalities.set(municipalityKey(row.C, row.D, row.E, row.G), cleanName(row.H));
  }
}

await writeFile(outputPath, renderSource(administrativeRegions, districts, municipalities));
console.log(
  `Wrote ${administrativeRegions.size} administrative regions, ${districts.size} districts, and ${municipalities.size} municipalities to ${path.relative(root, outputPath)}.`
);
...
function renderSource(administrativeRegions, districts, municipalities) {
  return `// Generated by scripts/generate-municipalities.mjs from docs/data/AuszugGV2QAktuell.xlsx.
